...
export default {
  ...
  methods: {
    ...
    async generateDocument(templatePath) {
      ...
      // O backend aceita ambos, mas é melhor usar apenas o nome para consistência
      ...
      try {
        await documentService.generateClientDocument(this.client.id, templateName)
    ...
    openEditor(templatePath) {
      // Extrair apenas o nome do arquivo se for um caminho relativo
      // O backend espera apenas o nome do arquivo (ex: "Contrato_Piso.docx")
      // não o caminho relativo (ex: "documents/Contrato_Piso.docx")
      let templateName = templatePath
      if (templatePath && templatePath.includes('/')) {
        // Se contém barra, extrair apenas o nome do arquivo
        templateName = templatePath.substring(templatePath.lastIndexOf('/') + 1)
      } else if (templatePath && templatePath.includes('\\')) {
        // Se contém barra invertida (Windows), extrair apenas o nome do arquivo
        templateName = templatePath.substring(templatePath.lastIndexOf('\\') + 1)
      }
